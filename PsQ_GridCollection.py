...
class GridCollection:     
    """     
    Container and processing hub for managing multiple related Sudoku grid collections, 
    with emphasis on dataset preparation for machine learning.

    A GridCollection instance maintains and coordinates **three distinct collections**:

    1. **Internal Collection (Immutable)**  
       - Set at construction time and **never altered** thereafter.  
       - Represents the *identity* of the GridCollection object.  
       - Typically contains valid Sudoku grids, often a single permutation series 
         or canonical form, serving as the reference for all derived datasets.

    2. **Active Series (Mutable)**  
       - Can be generated or updated from the internal collection via:
         - Geometric transformations (row/column permutations, box swaps).
         - Digit relabelings and recodings.
         - New grid generation (random valid grids, constrained puzzles, etc.).  
       - Serves as the primary working set for dataset creation and export.

    3. **Garbage Collection (Mutable)**  
       - Contains grids that are *not* part of the active series.  
       - May include:
         - Invalid Sudoku grids (rule violations, cardinality errors, structural conflicts).
         - Grids made invalid by minimal local changes.
         - Valid grids belonging to *different* permutation series.  
       - Often used for generating negative samples in binary classification datasets.

    **Supported Encodings:**
    - **Numeric** (digit-based integer representation).
    - **ABC-encoded** (canonical letter-mapping with fixed top row `A`–`I`).
    - **One-hot encoded** (for direct ML model input).

    **Key Capabilities:**
    - Apply transformation pipelines to active or garbage collections.
    - Recode or canonicalize grids via ABC-encoding.
    - Export collections to SQL databases or other string-based formats.
    - Generate binary, multiclass, or puzzle-based datasets with flexible class balance.
    - Split datasets into train/test partitions with optional stratification.

    **Primary Use Case:**
    Building structured Sudoku datasets for ML/DL models, with full control over 
    data generation, encoding, transformation, and export.

    Attributes
    ----------
    SIZE_internal : int
        Number of grids in the immutable internal collection.
    SIZE_activeSeries : int
        Number of grids currently in the active series.
    SIZE_garbageCollection : int
        Number of grids currently in the garbage collection.
    SIZE_labels : int
        Number of labels associated with the active series (if applicable).
    activationStatus : str
        Current mode or task type ('binary', 'multiclass', 'puzzle', etc.).
    prefix : str
        Prefix tag for identifying dataset/task variants.

    Notes
    -----
    - The immutable internal collection serves as a fixed point of reference, 
      ensuring reproducibility of derived datasets.
    - The active series and garbage collection are independent and may be 
      refreshed, repopulated, or cleared as needed.
    - The garbage collection is not inherently “bad data”; it’s a deliberately 
      maintained set of *out-of-class* or *off-distribution* grids.
    """        
